...
async function callAnalyzerModel(
  ...
  const target = resolveUpstream(modelSlot, {
    apiKey,
    openRouterPrefs,
    // included (DeepSeek) — analysis is non-reasoning by design, so the slot's own
    // id is ignored in favour of the non-thinking one.
  ...
  if (!response.ok) {
    throw classifyUpstreamFailure(response.status, await response.text(), 'Analyzer model error');
  }
