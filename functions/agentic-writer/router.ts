...
import { fetchStoryClicheIndex, formatClicheIndexForPrompt } from './cliche-index.ts';
...
import {
  ...
  isVeniceReasoningEffort,
  VENICE_REASONING_EFFORTS,
  type VeniceReasoningEffort,
...
function validateCommonBody(
  ...
  if (body['veniceEfforts'] !== undefined) {
    const efforts = body['veniceEfforts'];
    if (!efforts || typeof efforts !== 'object' || Array.isArray(efforts)
      || !Object.values(efforts).every(isVeniceReasoningEffort)) {
      return jsonResponse<ErrorResponse>(
        { error: `veniceEfforts must map model slots to one of: ${VENICE_REASONING_EFFORTS.join(', ')}` },
        400, headers,
      );
    }
  }
...
interface ApiAuthContext {
  userJwt: string;
  /** The author's OpenRouter key, from `X-API-Key`. */
  apiKey: string | null;
  /** The author's Venice key, from `X-Venice-Key`. */
  veniceApiKey: string | null;
}

/**
 * The key a given slot's provider needs, or null for one that needs none.
 *
 * Two remote providers now take the author's own key, and a pipeline may name BOTH in
 * different slots — a Venice writer with an OpenRouter refiner is a perfectly ordinary
 * setup. So the key cannot be chosen once per request; it is chosen per slot, here, at the
 * five call sites that were passing `authCtx.apiKey` unconditionally.
 *
 * `included` returns null on purpose: its key is ours, read from the environment inside
 * `resolveUpstream`, and must never come from a header.
 */
/**
 * The `reasoning_effort` the browser resolved for `slot` (#107), or undefined — which
 * `resolveUpstream` sends as `medium` for a Venice slot and ignores for every other provider.
 * Chosen per slot, like {@link keyForSlot}'s key, because a pipeline can name two different
 * Venice models in different slots, and each accepts its own levels.
 * `validateCommonBody` has already checked every value.
 *
 * **The lookup is case-sensitive** where everything else here lowercases the provider: a
 * `Venice:m` slot finds no `venice:m` entry. The client only mints lowercase `venice:` slots
 * today, and a miss degrades to `medium` rather than failing — but a future free-text slot
 * entry would need both sides to agree on the key.
 */
function effortForSlot(body: PhaseCommonBody, slot: string): VeniceReasoningEffort | undefined {
  return body.veniceEfforts?.[slot];
}

function keyForSlot(slot: string, auth: ApiAuthContext): string | null {
  switch (slotProvider(slot)) {
    case 'openrouter': return auth.apiKey;
    case 'venice': return auth.veniceApiKey;
    default: return null;
  }
}
...
function checkProviderKeys(
  ...
  const veniceApiKey = request.headers.get('X-Venice-Key') || null;
  const slots = [models.writing, models.research, models.refiner, models.analyzer];

  if (slots.some(m => slotProvider(m) === 'openrouter') && !apiKey) {
    return jsonResponse<ErrorResponse>({ error: 'X-API-Key header required for OpenRouter models' }, 400, headers);
  }
  if (slots.some(m => slotProvider(m) === 'venice') && !veniceApiKey) {
    return jsonResponse<ErrorResponse>({ error: 'X-Venice-Key header required for Venice models' }, 400, headers);
  }
  return { userJwt, apiKey, veniceApiKey };
}
...
// `extractAuthFromRequest` has actually verified the JWT — `checkProviderKeys`
...
export async function handlePlan(
  ...
  const authCtx = checkProviderKeys(request, body.models, headers);
  ...
  try {
    ...
    const result = await callModel(
      ...
      planningInput,
      { ...planningConfig, reasoningEffort: effortForSlot(body, body.models.writing) },
      keyForSlot(body.models.writing, authCtx),
  ...
  try {
    ...
    if (plan.tasks.length > 0) {
      ...
      const briefs = await runResearchAgents(
        ...
        keyForSlot(body.models.research, authCtx),
        body.openRouterPrefs,
        wd.signal,
        effortForSlot(body, body.models.research),
      );
  ...
  (async () => {
    try {
      ...
      const result = await streamToClient(
        ...
        '',
        enrichedMessages,
        { ...draftConfig, reasoningEffort: effortForSlot(body, body.models.writing) },
        keyForSlot(body.models.writing, authCtx),
        body.openRouterPrefs,
        timeoutMs,
        wd.signal,
      ...
      if (draftTruncated) {
        // `maxTokensForSlot`, not the raw config: a Venice slot is sent a padded ceiling, and
        // logging the unpadded one understates the real cap while triaging a truncation.
        console.warn(`[AgenticWriter] Draft truncated: finish_reason=length, wordCount=${wordCount}, maxTokens=${maxTokensForSlot(body.models.writing, draftConfig.maxTokens)}, outputTokens=${result.outputTokens}`);
      }
      if (
...
export async function handleAnalyze(
  ...
  try {
    ...
    const result = await callModel(
      ...
      ANALYZER_SYSTEM_PROMPT,
      analyzerUserContent,
      { ...analyzerConfig, reasoningEffort: effortForSlot(body, body.models.analyzer) },
      keyForSlot(body.models.analyzer, authCtx),
      body.openRouterPrefs,
      TIMEOUT_ANALYZER_MS,
      wd.signal,
    );
    ...
    try {
      ...
      try {
        const refineStreamResult = await streamToClient(
          ...
          '',
          refineMessages,
          { ...refineConfig, reasoningEffort: effortForSlot(body, body.models.refiner) },
          keyForSlot(body.models.refiner, authCtx),
          body.openRouterPrefs,
          TIMEOUT_REFINE_MS,
          wd.signal,
        );
        ...
        if (refineStreamResult.content.trim().length === 0) {
          ...
          console.warn(`[AgenticWriter] Refine truncated: finish_reason=length, wordCount=${wordCount}, maxTokens=${maxTokensForSlot(body.models.refiner, refineConfig.maxTokens)}, draftLength=${body.draftContent.length}, outputTokens=${refineStreamResult.outputTokens}`);
