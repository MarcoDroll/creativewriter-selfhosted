...
// ---------------------------------------------------------------------------
// 0 — `veniceEfforts` validation (#107). Here because this is the file that drives
// `handleRequest`, and `validateCommonBody` is only reachable through it.
// ---------------------------------------------------------------------------

for (const [label, efforts] of [
  ['an unknown level', { 'venice:m': 'turbo' }],
  ['an array instead of a map', ['low']],
] as const) {
  Deno.test(`/plan: veniceEfforts with ${label} is a 400, with no upstream call (#107)`, async () => {
    await withEnv(SELF_HOSTED_BASE, async () => {
      await withFetch(completion, async (calls) => {
        const token = await mintToken(`local-cfg-venice-effort-${label.length}`);
        const response = await handleRequest(phaseRequest(
          '/plan', token, { ...COMMON, models: { writing: 'openrouter:x/y' }, veniceEfforts: efforts }, 'sk-or',
        ));
        assertEquals(response.status, 400);
        assertStringIncludes((await response.json() as ErrorBody).error, 'veniceEfforts');
