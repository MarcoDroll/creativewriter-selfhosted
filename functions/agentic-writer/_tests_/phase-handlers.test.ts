 ...
 *
 * `/analyze-cliches` is the one route with no file here, because it is handled inline in
 * the router and so has nothing to call below the gate. Its own file,
 * `analyze-cliches.test.ts`, signs a token and serves the JWKS document off the same stub
 * — a third upstream, not a different tool — and is where `extractAuthFromRequest` itself
 * is covered.
 */
...
function draftBody(): DraftRequestBody {
  return { ...COMMON, messages: MESSAGES, researchContext: '', clicheBlock: '' } as unknown as DraftRequestBody;
}
