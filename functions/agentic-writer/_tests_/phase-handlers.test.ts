...
import type { VeniceReasoningEffort } from '../../_shared/model-calling.ts';
import type {
  AnalyzeRequestBody,
  DraftRequestBody,
  PlanRequestBody,
...
// ---------------------------------------------------------------------------
// #107 — every phase sends Venice the effort the browser resolved for ITS slot
// ---------------------------------------------------------------------------

/** Four Venice slots with four different levels, so a phase reading the wrong slot shows. */
const VENICE_MODELS = { writing: 'venice:w', research: 'venice:r', refiner: 'venice:f', analyzer: 'venice:a' };
const VENICE_EFFORTS: Record<string, VeniceReasoningEffort> =
  { 'venice:w': 'low', 'venice:r': 'minimal', 'venice:f': 'high', 'venice:a': 'max' };

const withVenice = <T>(body: T): T => ({ ...body, models: VENICE_MODELS, veniceEfforts: VENICE_EFFORTS }) as T;
const veniceRequest = () => keyedRequest({ venice: 'vk' });

function analyzeBody(): AnalyzeRequestBody {
  return { ...COMMON, messages: MESSAGES, draftContent: 'a draft', clicheBlock: 'CLICHÉ INDEX' } as unknown as AnalyzeRequestBody;
}

/**
 * The `reasoning_effort` of every model call `run` makes. PostgREST answers empty (for
 * /research); the model answers 401, which ends every phase after one call per agent. The
 * response is drained because /draft and /refine only call the model inside their stream.
 */
async function effortsSent(run: () => Promise<Response>): Promise<unknown[]> {
  const sent: unknown[] = [];
  const realFetch = globalThis.fetch;
  const saved = ['SELF_HOSTED', 'SUPABASE_URL', 'SUPABASE_ANON_KEY'].map(k => [k, Deno.env.get(k)] as const);
  Deno.env.delete('SELF_HOSTED');
  Deno.env.set('SUPABASE_URL', 'http://localhost:54321');
  Deno.env.set('SUPABASE_ANON_KEY', 'test-anon-key');
  globalThis.fetch = (input: string | URL | Request, init?: RequestInit) => {
...
Deno.test('/plan sends the writing slot\'s effort (#107)', async () => {
  assertEquals(await effortsSent(() => handlePlan(veniceRequest(), withVenice(planBody()), HEADERS)), ['low']);
});

Deno.test('/research sends the research slot\'s effort, once per agent (#107)', async () => {
  assertEquals(
    await effortsSent(() => handleResearch(veniceRequest(), withVenice(researchBody()), HEADERS)),
    ['minimal', 'minimal'],
  );
});

Deno.test('/draft sends the writing slot\'s effort (#107)', async () => {
  assertEquals(await effortsSent(() => handleDraft(veniceRequest(), withVenice(draftBody()), HEADERS)), ['low']);
});

Deno.test('/analyze sends the analyzer slot\'s effort (#107)', async () => {
  assertEquals(await effortsSent(() => handleAnalyze(veniceRequest(), withVenice(analyzeBody()), HEADERS)), ['max']);
});

Deno.test('/refine sends the refiner slot\'s effort (#107)', async () => {
  assertEquals(await effortsSent(() => handleRefine(veniceRequest(), withVenice(refineBody()), HEADERS)), ['high']);
});
