...
Deno.test('a Venice research agent sends the effort the browser resolved (#107)', async () => {
  ...
  // pass the level to `resolveUpstream` itself — and it runs up to five rounds at 800 tokens.
  const sent: unknown[] = [];
  const realFetch = globalThis.fetch;
  globalThis.fetch = (_input: string | URL | Request, init?: RequestInit) => {
  ...
  try {
    await runResearchAgents(
      'venice:deepseek-v4-flash-0731', [TASKS[0]], new Map(), '', { codexEntries: [], scenes: [] },
      'vk', undefined, undefined, 'low',
    );
  } finally {
    globalThis.fetch = realFetch;
  }
