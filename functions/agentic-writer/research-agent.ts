...
import { maxTokensForSlot, parseModelSlot, resolveUpstream, ModelProviderConfigError } from '../_shared/model-calling.ts';
...
export async function runResearchAgent(
  ...
  signal?: AbortSignal,
  /** Venice only — see `resolveUpstream`'s option of the same name (#107). */
  ...
  const target = resolveUpstream(modelSlot, { apiKey, openRouterPrefs, veniceReasoningEffort });
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS + 1; round++) {
    ...
    const postRound = (): Promise<Response> => {
      const body: Record<string, unknown> = {
        model: modelId,
        messages,
        // Through the helper, not raw: this body is built here rather than by `callModel`,
        // so it does not inherit the Venice reasoning headroom automatically. Research is
        // the phase most likely to time out and runs up to 5 rounds per task, so an 800-token
        // budget eaten by hidden thinking is the worst place to leave unpadded.
        max_tokens: maxTokensForSlot(modelSlot, agentConfig.maxTokens),
