...
/**
 * Atomic flat-cost increment for an included image generation.
 *
 * Mirrors {@link logUsage} but charges a fixed per-image cost ({@link IMAGE_COST_USD})
 * instead of token-based pricing. The `increment_ai_usage` RPC bumps `request_count`
 * by exactly 1 per call, so `count` only scales the cost — call once per generated
 * batch (we currently always generate a single image, so count is 1).
 */
