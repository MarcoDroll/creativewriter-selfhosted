 ...
 *
 * Requires the subscription to be fetched with expand: ['data.items.data.price.product'].
 */
function extractSubscriptionData(sub: Stripe.Subscription, logPrefix = ''): SubscriptionData {
  ...
  if (tier === 'none' && priceId) {
    ...
    const productName = typeof product === 'object' && product !== null && 'name' in product
      ? (product as { name: string }).name : undefined;
    if (productName) {
...
export async function syncStripeData(
  ...
  // The expand option inlines the product object so the tier fallback costs no extra API call.
  ...
  const subData = subscriptions.data.length === 0
    ...
    : extractSubscriptionData(subscriptions.data[0]);
...
export async function validateJwtAndGetSubscription(
  ...
  if (!customerId) {
    ...
    if (existing.data.length > 0) {
      ...
      // App trial: grant basic access for new users within the trial window
  ...
  if (cached) {
    ...
    try {
      ...
      if (subscriptions.data.length === 0) {
        ...
        subData = extractSubscriptionData(subscriptions.data[0], ' (direct)');
      }
    }
  }
  ...
  return { valid: isActive, email, tier, subData, customerId, userId };
}
