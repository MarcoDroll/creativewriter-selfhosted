...
import { persistSelfHostedLicenseValidation, validateLicenseKey } from './license.ts';
...
export async function validateJwtAndGetSubscription(
  ...
  if (licenseKey) {
    ...
    if (licenseResult.valid) {
      // Durable for cw_current_tier() (#187) — but ONLY on a genuine self-hosted runtime.
      // Gated the same way the hosted-trust gate just below is (secret possession, not a
      // self-declared flag): a leaked license key already lets any caller's /stripe/verify
      // response claim tier:'premium' for this one request regardless of instance, but
      // persisting that into a row a SQL trigger trusts for up to a year must never be
      // reachable on the real hosted database, where subscription_cache is the actual
      // authority. See persistSelfHostedLicenseValidation's own doc comment.
      if (!(await isGenuineHostedInstance())) {
        await persistSelfHostedLicenseValidation(userId, licenseResult.tier, licenseResult.expiresAt);
      }
      return {
        valid: true,
        email,
