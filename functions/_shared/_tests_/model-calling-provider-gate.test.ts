...
Deno.test('allowedSlotProviders: hosted is the three remote providers', () => {
  // `venice` joined `openrouter` and `included` when the function learned to call
  // api.venice.ai with the key the client forwards as `X-Venice-Key`.
  withSelfHosted(undefined, () => {
    assertEquals([...allowedSlotProviders()].sort(), ['included', 'openrouter', 'venice']);
  });
...
Deno.test('allowedSlotProviders: self-hosted adds the two local providers, and KEEPS included', () => {
  withSelfHosted('true', () => {
    assertEquals(
      [...allowedSlotProviders()].sort(),
      ['included', 'ollama', 'openaicompatible', 'openrouter', 'venice'],
    );
  });
...
Deno.test('checkSlotProviders: hosted accepts the hosted providers', () => {
  withSelfHosted(undefined, () => {
    assertEquals(checkSlotProviders(FOUR_SLOTS), null);
    assertEquals(checkSlotProviders({ ...FOUR_SLOTS, writing: 'venice:venice-uncensored' }), null);
  });
});
...
// The single-key shape, which `/analyze-cliches` used before that route was deleted. Kept
// because `checkSlotProviders` still iterates whatever keys it is handed, and a one-key map is
// the boundary case its loop is easiest to break on.
Deno.test('checkSlotProviders: a single-key slot map', () => {
  withSelfHosted(undefined, () => {
