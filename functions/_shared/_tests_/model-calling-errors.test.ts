...
Deno.test('callModel and streamToClient send a Venice slot the configured effort (#107)', async () => {
  // Not an error path, but the only file that drives both callers directly: each builds its own
  // body and has to hand `config.reasoningEffort` to `resolveUpstream`. The 401 just ends the
  // call once the body has been seen.
  const sent: unknown[] = [];
  const realFetch = globalThis.fetch;
  ...
  try {
    const config = { ...CONFIG, reasoningEffort: 'low' as const };
    await assertRejects(() => callModel('venice:m', 'sys', 'user', config, 'vk'), UpstreamError);
    const writer = nullWriter();
    await assertRejects(() => streamToClient(writer, 'venice:m', 'sys', 'user', null, config, 'vk'), UpstreamError);
    await writer.close().catch(() => { /* nothing read it */ });
  } finally {
    globalThis.fetch = realFetch;
  }
  assertEquals(sent, ['low', 'low']);
});
