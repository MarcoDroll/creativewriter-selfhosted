...
Deno.test('resolveUpstream: venice carries the forwarded key, and medium when no effort was named', () => {
  // The author's own key, exactly as OpenRouter's is. The one body extra is the effort the
  // browser sends too (#107) — unset, the browser's own fallback. Still no
  // `venice_parameters`: the same model must answer the same way whichever surface called it.
  ...
  assertEquals(target.bodyExtras, { reasoning_effort: 'medium' });
});

Deno.test('resolveUpstream: venice sends the effort the browser resolved for the slot (#107)', () => {
  // DeepSeek maps a `medium` it does not offer to `high`, so the browser's resolved `low` is
  // what actually caps a 600-token analyzer.
  const target = resolveUpstream('venice:deepseek-v4-flash-0731', { apiKey: 'vk', veniceReasoningEffort: 'low' });
  assertEquals(target.bodyExtras, { reasoning_effort: 'low' });
});

Deno.test('resolveUpstream: no other provider is sent a reasoning_effort', () => {
  const target = resolveUpstream('openrouter:x/y', { apiKey: 'k', veniceReasoningEffort: 'low' });
  assertEquals('reasoning_effort' in target.bodyExtras, false);
});
