...
| 400 *"Model provider … is not available on this instance"* | A slot names a provider the server cannot call — a `gemini:`/`claude:` model, or a local one on the hosted service | Re-pick the slot in **Settings → Deep Writer** |
