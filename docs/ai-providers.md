...
- **Hosted SaaS (creativewriter.dev) or self-hosted behind an HTTPS reverse proxy:** browser mixed-content rules block plain HTTP requests from an HTTPS page, regardless of CSP. Expose Ollama over HTTPS via a tunnel (Cloudflare Tunnel, ngrok) or an nginx reverse proxy with a TLS cert.
