...
--
-- This file represents the schema after migrations 00001–00009. It is
-- executed by Postgres' initdb phase on first boot of a fresh self-hosted
-- install, so the migration runner (migrate.sh) skips those migrations
-- via a seed list in init/zz-bootstrap.sh.
--
-- IMPORTANT: if you change this file to reflect a *later* migration
-- (e.g. fold 00010 in here), you MUST also extend the seed list in
-- BOTH init/zz-bootstrap.sh AND migrate.sh to match — otherwise the
-- runner will try to re-apply already-baked migrations on fresh installs.
-- Skipping that step is exactly how issue #11 (00009 fails on fresh
-- install) was introduced.
