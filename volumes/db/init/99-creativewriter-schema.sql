...
-- 3. ai_usage_daily — replaces KV `usage:{customerId}:{date}` entries
CREATE TABLE IF NOT EXISTS ai_usage_daily (
  ...
  CONSTRAINT ai_usage_daily_customer_date_key UNIQUE (stripe_customer_id, usage_date)
);
...
-- and (stripe_customer_id, usage_date). No additional indexes needed.
...
BEGIN
  INSERT INTO ai_usage_daily (stripe_customer_id, usage_date, total_cost_usd, request_count, input_tokens, output_tokens, last_updated)
  VALUES (p_customer_id, p_date::DATE, p_cost, 1, p_input_tokens, p_output_tokens, now())
  ON CONFLICT (stripe_customer_id, usage_date)
  DO UPDATE SET
...
-- Auto-cleanup: delete ai_usage_daily rows older than 30 days
-- This can be run via pg_cron or manually
CREATE OR REPLACE FUNCTION cleanup_old_ai_usage() RETURNS void AS $$
BEGIN
  DELETE FROM ai_usage_daily WHERE usage_date < CURRENT_DATE - INTERVAL '30 days';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
