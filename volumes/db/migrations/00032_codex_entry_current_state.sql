...
create table if not exists public.codex_entry_current_state (
...
create index if not exists idx_cecs_story_id on public.codex_entry_current_state(story_id);
create index if not exists idx_cecs_user_id on public.codex_entry_current_state(user_id);
...
drop policy if exists "Users own their current-state rows" on public.codex_entry_current_state;
