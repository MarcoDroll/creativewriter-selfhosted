...
--
-- All DROP POLICY statements use IF EXISTS so the migration is idempotent
-- and safe on fresh self-hosted installs. The two storage.objects policies
-- in particular are created by 99-storage-setup.sql via the storage-init
-- service, which runs in parallel with the migrate runner — without
-- IF EXISTS, migrate would race storage-init and abort here.
--
...
DROP POLICY IF EXISTS "Users own their stories" ON public.stories;
CREATE POLICY "Users own their stories"
  ON public.stories FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their chapters" ON public.chapters;
CREATE POLICY "Users own their chapters"
  ON public.chapters FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their scenes" ON public.scenes;
CREATE POLICY "Users own their scenes"
  ON public.scenes FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their codexes" ON public.codexes;
CREATE POLICY "Users own their codexes"
  ON public.codexes FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their codex categories" ON public.codex_categories;
CREATE POLICY "Users own their codex categories"
  ON public.codex_categories FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their codex entries" ON public.codex_entries;
CREATE POLICY "Users own their codex entries"
  ON public.codex_entries FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their scene chats" ON public.scene_chats;
CREATE POLICY "Users own their scene chats"
  ON public.scene_chats FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their character chats" ON public.character_chats;
CREATE POLICY "Users own their character chats"
  ON public.character_chats FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their story research" ON public.story_research;
CREATE POLICY "Users own their story research"
  ON public.story_research FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their story snapshots" ON public.story_snapshots;
CREATE POLICY "Users own their story snapshots"
  ON public.story_snapshots FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their beat histories" ON public.beat_histories;
CREATE POLICY "Users own their beat histories"
  ON public.beat_histories FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their story images" ON public.story_images;
CREATE POLICY "Users own their story images"
  ON public.story_images FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their story videos" ON public.story_videos;
CREATE POLICY "Users own their story videos"
  ON public.story_videos FOR ALL
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users own their custom backgrounds" ON public.custom_backgrounds;
...
DROP POLICY IF EXISTS "Users access own story media" ON storage.objects;
...
DROP POLICY IF EXISTS "Users access own backgrounds" ON storage.objects;
