...
CREATE TABLE IF NOT EXISTS public.cliche_phrases (
...
DROP POLICY IF EXISTS "Authenticated users can read cliche phrases" ON public.cliche_phrases;
CREATE POLICY "Authenticated users can read cliche phrases"
  ON public.cliche_phrases FOR SELECT TO authenticated USING (true);

CREATE INDEX IF NOT EXISTS idx_cliche_phrases_lang_cat ON public.cliche_phrases(language, category);
-- Unique constraint enables idempotent re-seeding via ON CONFLICT below.
CREATE UNIQUE INDEX IF NOT EXISTS idx_cliche_phrases_unique
  ON public.cliche_phrases(language, category, phrase);

-- ---------------------------------------------------------------------------
-- Table 2: story_cliche_index (per-story detections + user entries)
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS public.story_cliche_index (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP POLICY IF EXISTS "Users own their story cliche index" ON public.story_cliche_index;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_story_cliche_unique_phrase
  ON public.story_cliche_index (story_id, lower(phrase));

-- RLS performance index
CREATE INDEX IF NOT EXISTS idx_story_cliche_index_user ON public.story_cliche_index(user_id);
...
ON CONFLICT (language, category, phrase) DO NOTHING;
