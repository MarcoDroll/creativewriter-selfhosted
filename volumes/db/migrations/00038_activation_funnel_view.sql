...
-- extend the window.
--
-- Replay-safety: DROP VIEW IF EXISTS + CREATE (NOT create-or-replace). A later
-- migration (00046) reshapes this view by inserting columns in the middle; on a
-- recovery replay this migration re-runs against that rev-2 shape, and
-- CREATE OR REPLACE VIEW only allows appending columns at the end ("cannot drop
-- columns from view"). Dropping first makes the second apply a clean re-run.
