...
# Mark this instance as self-hosted, so SQL can tell.
#
# The Roleplay tier caps (00067) read `cw_instance_config` to decide a caller's allowance. On
# hosted, tier comes from `subscription_cache`, which only the Stripe webhook writes; a self-hosted
# install has no Stripe and no row, so without this flag it would resolve to 'unknown' and every
# self-hoster would silently lose the Basic allowance they are meant to have.
#
# **Seeded here and NOT in init/zz-bootstrap.sh**, deliberately: bootstrap runs during initdb, i.e.
# only when the database is first created, so an existing install upgrading into 00067 would never
# receive the flag. This runner executes on every `docker compose up`, which covers fresh installs
# and upgrades alike.
#
# Guarded on the table existing, because this runs unconditionally — including against a database
# whose migrations have not yet reached 00067. `|| true` and ON CONFLICT keep it from tripping the
# ERR trap under `set -euo pipefail`; a failure to mark the instance must never take container
# start down.
has_instance_config=$(psql -tAc "SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cw_instance_config'" 2>/dev/null || echo "")
if [ "$has_instance_config" = "1" ]; then
  psql -q <<'SQL' || true
INSERT INTO public.cw_instance_config (key, value) VALUES ('self_hosted', 'true')
